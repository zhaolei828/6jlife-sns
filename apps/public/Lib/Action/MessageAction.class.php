...
class MessageAction extends Action
{
	...
	public function loadMessage() {
		...
		// $imgUrl = array();
		// foreach ($message['data'] as $value) {
		// 	if ($value['attach_type'] === 'message_image') {
		// 		foreach ($value['attach_infos'] as $k => $v) {
		// 			$imgUrl[$value['message_id']][$v['attach_id']] = array('length'=>count($value['attach_infos']), 'index'=>$k + 1, 'url'=>$v['url'], 'attach_id'=>$v['attach_id']);
		// 		}
		// 	}
		// }
